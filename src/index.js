...
function resolveMalaysia(text){
 ...
 let m=low.match(/(?:rm|myr)\s*(\d[\d,]*(?:\.\d+)?)\s*(k|ribu|juta|m)?/);
 if(m){let amount=parseFloat(m[1].replaceAll(",",""));if(["k","ribu"].includes(m[2]))amount*=1000;if(["juta","m"].includes(m[2]))amount*=1000000;resolved.amount=amount;resolved.currency="MYR";}
 ...
 m=text.match(/(?<!\d)(\d{12})(?!\d)/);if(m){resolved.registration_no=m[1];warnings.push("SSM number parsed by format only; registration existence not verified.");}
