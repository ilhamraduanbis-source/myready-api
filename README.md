...
Every successful machine-ready operation is assigned RM0.01 of simulated economic value. Qualifying operations are recorded once in the production D1 ledger using their operation ID and repeated requests are marked as duplicates. This is usage metering, not revenue: pilot responses state `economic_mode: simulated_value_only` and `cash_collected: false`. The ledger deliberately stores no source text and real collection is disabled.
...
Only locally ready operations qualify for simulated value. A `needs_review` result can still be locally ready when the remaining warnings require an authoritative MyInvois check. Rejected and duplicate operations do not add simulated value.
